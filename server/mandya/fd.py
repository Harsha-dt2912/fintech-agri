...
# Define sample villages near Mandya and business types
villages_near_mandya = [
...
num_rows = 100  # Number of rows you want to generate

for i in range(num_rows):
    while True:
        area = random.choice(villages_near_mandya)
        ...
        if area not in area_business_population:
            area_business_population[area] = random.randint(1000, 10000)
        ...
        num_businesses = random.randint(2, 30)
        need = round(random.uniform(40, 80), 2)
        ...
        if (num_businesses, need) not in business_unique_values[(area, business)]:
            ...
            combination = (area, population, business, num_businesses, need, f"{success_rate}%")
...
# Create a DataFrame
columns = ["Area Name", "Population", "Business Name", "Number of Businesses", "Need", "Success Rate (%)"]
df = pd.DataFrame(data, columns=columns)

file_path = "villages_near_mandya_data.csv"
...
clean_file_path = "cleaned_sorted_villages_near_mandya_data.csv"
